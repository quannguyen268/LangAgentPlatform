...
### Design decisions
1. **No double-counting.** `stream_mode="values"` yields cumulative message snapshots. `_run_segment` tracks a `counted` index (initialized to the echo chunk's message count) and only records usage for `messages[counted:]` each chunk — each `AIMessage` is costed exactly once, across both the segment and the outer loop's carried-forward history.
